...
| `right_click`  | `target` | open the target's context menu |
| `drag`         | `from`, `to` | drag between resize-aware window points |
...
| `wait_until`   | `target`, `timeout_ms?`, `poll_ms?` | poll until text/AX target appears |

`wait_until` observes immediately, then polls until the target resolves or
`timeout_ms` expires (default 2500ms). Prefer it over `wait` when the UI has a
visible or accessibility-backed readiness signal: a fast UI proceeds without
paying the full timeout, while a slow UI remains bounded.
...
Actions that address a location or condition (`click`, `double_click`, `right_click`, `drag`, `move`,
`wait_until`, `assert`) take a `target`. Three forms, in order of preference:
