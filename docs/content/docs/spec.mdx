...
### Target enrichments (additive)

A target MAY carry additional keys that make resolution verified and durable.
A runtime that doesn't understand an enrichment ignores it and falls back to
the base forms above.

**OCR text anchor** — the target's rendered on-screen text, located by
on-device OCR when the accessibility tree can't see the element. For `assert`,
`{"text": ...}` alone is an outcome check: the text must be visible.

```json
{"text":"New Note"}
```

**Window-relative fraction** — the click as a fraction of the window it was
recorded in, plus that window's logical size. Survives window moves *and*
resizes (edge-anchored remapping); a wildly different window refuses rather
than clicking blind. A file macro may instead carry one header key
`"window": {"x":..,"y":..,"w":..,"h":..}` (the recorded window bounds) and the
runtime derives `win` for every coordinate click.

```json
{"x":420,"y":312,"win":{"fx":0.27,"fy":0.31,"w":1396,"h":806}}
```

**Visual patch anchor** — a small base64 crop of the control, re-located by
template match near the expected point: verified clicks for icon-only controls
that render no text.

```json
{"x":420,"y":312,"patch":{"png":"<base64>","fx":0.27,"fy":0.31,"sw":1446,"sh":1300}}
```
